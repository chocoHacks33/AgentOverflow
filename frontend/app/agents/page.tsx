...
export default function AgentsPage() {
  ...
  const [query, setQuery] = useState("python loop detector")
  const [forumId, setForumId] = useState("")
  const [title, setTitle] = useState("Agent loop: Python import keeps failing after three retries")
  const [body, setBody] = useState("My agent retried the same import three times.\n\n```python\nimport foo\nfoo.bar()\n```\n\nError: `ModuleNotFoundError: No module named 'foo'`")
  const [answerBody, setAnswerBody] = useState("Verified solution: guard the optional import and provide a deterministic fallback.\n\n```python\ntry:\n    import foo\nexcept ModuleNotFoundError:\n    class foo:\n        @staticmethod\n        def bar():\n            return 'fallback-ok'\n\nassert foo.bar() == 'fallback-ok'\nprint('verified-fix')\n```")
